...
- [x] 6. 蜡封 4 种 + 标准包（commit ff6f0d02）
- [x] 7. 幽灵牌 18 张 + 幽灵包 + 回填 4 张小丑（commit 22f6b010）
- [x] 收尾：`/code-review high` + 修三个洞（commit 4c7c8e77）
...
七步全部落地，**718 个测试绿**，`npm run build` 通。
覆盖面：小丑 **124 / 150**、消耗品 **52 / 52**、补充包 **5 / 5**。
细节在 [17 号票](../.scratch/balatro-复刻/issues/17-补充包与幽灵牌的切片边界.md) 的「交付结果」。

**破墙实测**：贪心深度 2.875 → 3.0（消耗品）→ **3.375**（补充包），最深 Ante 5。
**墙松了两格，还是没破。** 但「还差多少内容」现在**测不出来**——
那个贪心不挑手牌，所以 13 张塔罗与全部加蜡封的幽灵牌都用不出来。
→ **下一步先写一个会挑牌的策略**，再谈内容缺口。

**下一刀（内容侧）最划算的是强化牌那 9 张小丑**：
`coverage.test.ts` 里最大的一组，前置（8 种强化）全落地了。
`增删牌` 那 8 张现在**也不阻塞了**——标准包与造牌幽灵牌把
「运行时造扑克牌」铺好了，`removeCards` 也有了。
