...
- [实机对拍通道](issues/21-实机对拍通道.md) —— **用户在雷电模拟器里装了同构建的正版，adb 驱动它取真值**。
  第一局就抓到一个 bug：**开局造牌的规范序错了**（`game.lua:2584` 按 `花色..点数` 字符串排序，
  点数是 `2..9 A J K Q T`），首手 3 张错位；修掉之后首手、12 张补牌、首个商店、标准包 5 张全对。
  04 号票的结构性验证与复刻件共用这个假设，所以一直是绿的。
  把复刻件放进模拟器浏览器后又看出两处：**红牌组的 +1 弃牌从没加过**（`back.lua:211`）、
  **手牌区摸牌后不排序**（原作每摸一张 `G.hand:sort()`，而手牌区顺序决定计分次序）。
  **所有深度快照换了一批数，新旧不可比**；60 seed 上各条结论复核后方向不变。
...
> **（2026-09-21 已修，22 号票第十二步**：商店里的牌堆在 Cash Out 时 `shuffle('cashout'..ante)`，发的手牌实机逐张对上；
> 60 seed 贪心 2.367 → 2.767、挑牌 3.767 → 3.95，各条结论方向不变。）
>
...
>
> **18 号票第 4 步也做了，结论是不跳。** 两轮实验（120 / 240 seed）试了九种跳过策略，
> 没有一种比不跳好（120 seed：全跳 2.517、只为开包标签跳 3.975，不跳 4.008）；
> 唯一打平的「上一关赢 5 倍以上才跳」平均每局只跳 0.1 次。
> 原因与存利息相同：bot 缺的是战力，跳过少一次商店。策略参数留着，默认不跳。
>
> **下一刀的判断**：小丑、消耗品、补充包、标签都做完了。剩下的大块是**优惠券**（商店第五格，
> 16 / 17 / 18 号票都推迟过它，Voucher Tag 也等着它）与**其它牌组**（现在只有红牌组）。
> 墙那边，存利息、跳盲注、那 9 张强化牌小丑的零贡献，指向同一个结论：挑牌 bot 缺的是战力，
> 而它最大的短板还是**估值只认计分管线里的成长**。
...
- **洗牌顺序有外部真值了**（21 号票，`emulator-truth.test.ts`）。此前只有结构性验证，
  而它与复刻件共用「开局造牌的顺序」这个假设，**两边一起错了很久**。
  教训：结构性验证挡不住共同假设，能拿实机真值的地方就拿。
- **开局造牌的顺序是字符串字节序**（`game.lua:2584`）：`C2..C9, CA, CJ, CK, CQ, CT, D2, …`。
  与 `front` 抽 `P_CARDS` 的顺序是同一种字节序，但**是两处独立的代码**，别以为改了一处另一处就对了。
- **复刻件没有「牌组」概念，红牌组是在 `Run.back` 里硬编码的。** 做其它牌组时从 `back.lua:Back:apply_to_run` 抄全。
- **手牌区的顺序是逻辑状态**：摸牌后按 `G.hand.config.sort` 重排（`Round.sortHand`），
  而 `emplace` 进手牌区（DNA / Certificate / 造牌幽灵牌）**不排**——原文如此。
- **驱动模拟器的操作坑见 21 号票**：档要先走完教程（否则首个商店、优惠券、标签被强制）、
  敲字要逐字、点按要按 120ms、买东西是长距离慢拖。
