...
**产物不入库**（`.gitignore` 的 `**/报告/*.html`），要发人的时候现跑。
原因：这个脚本只打包 SRC 这一篇，而仓库里已有 12 篇分析文档；它也不在
`balatro_all.py` 的链路里，没人会记得重跑。一份已提交的陈旧 HTML 比没有更糟——
读的人分辨不出它是旧的。要扩成全量报告的话，改 SRC 为文档列表并挂进 all 链路。
