...
export(float) var projectile_spread := 0
...
func get_bouncing_text(base_stats: Resource) -> String:
	...
	return "\n" + Text.text("PIERCING_FORMATTED", [col_a + tr("BOUNCE") + col_b, a + str(bounce) + col_b, aa + str(round(bounce_dmg_reduction * 100)) + col_b])
...
func get_piercing_text(base_stats: Resource) -> String:
	...
	return "\n" + Text.text("PIERCING_FORMATTED", [col_a + tr("PIERCING") + col_b, a + str(piercing) + col_b, aa + str(round(piercing_dmg_reduction * 100)) + col_b])
...
func get_base_cooldown_value(base_stats: Resource) -> float:
	...
	return base_shooting_data.get_shooting_total_duration() + base_stats.cooldown / 60


func get_cooldown_value(_base_stats: Resource) -> float:
	var current_shooting_data = RangedShootingData.new(self)
	return current_shooting_data.get_shooting_total_duration() + cooldown / 60
