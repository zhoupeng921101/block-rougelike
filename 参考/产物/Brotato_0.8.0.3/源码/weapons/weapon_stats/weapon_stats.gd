...
export(float, 0, 1, 0.05) var accuracy = 1
export(float, 0, 1, 0.01) var crit_chance = 0.03
...
export(float, 0, 1, 0.1) var effect_scale := 1
export(Array, Array) var scaling_stats = [["stat_melee_damage", 1]]
export(float, 0, 1, 0.01) var lifesteal := 0
...
export(float) var additional_cooldown_multiplier = -1
...
func get_crit_chance_text(base_stats: Resource) -> String:
	...
	return a + str(max(crit_chance * 100, 0)) + col_b


func get_cooldown_text(base_stats: Resource, multiplier: float = 1) -> String:
...
func get_lifesteal_text(base_stats: Resource) -> String:
	...
	return "\n" + Text.text("STAT_FORMATTED", [col_a + tr("LIFESTEAL") + col_b, a + str(round(lifesteal * 100))]) + "%" + col_b


func get_accuracy_text(base_stats: Resource) -> String:
	var a = get_col_a(accuracy, base_stats.accuracy)
	var text = a + str(clamp(accuracy * 100, 0, 100)) + "%" + col_b

	if accuracy != base_stats.accuracy:
		text += init_a + str(base_stats.accuracy * 100) + "%" + col_b
