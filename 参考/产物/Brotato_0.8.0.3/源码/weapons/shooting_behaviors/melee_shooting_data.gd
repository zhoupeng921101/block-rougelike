...
func _init(from_stats: Resource = null, use_global_stats: bool = true) -> void:
	atk_spd = (Utils.get_stat("stat_attack_speed") + from_stats.attack_speed_mod) / 100 if use_global_stats else 0
...
func get_range_factor(distance: float) -> float:
	return max(0, distance / clamp(70 * (1 + (atk_spd / 3)), 70, 120))


func get_atk_duration() -> float:
	return max(0.01, BASE_ATK_DURATION - (atk_spd / 10)) + range_factor * 0.15
