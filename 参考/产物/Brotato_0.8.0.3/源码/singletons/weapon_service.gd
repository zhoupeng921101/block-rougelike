...
func init_melee_stats(from_stats: MeleeWeaponStats = MeleeWeaponStats.new(), weapon_id: String = "", sets: Array = [], effects: Array = [], is_structure: bool = false) -> MeleeWeaponStats:
	...
	if not is_structure:
		new_stats.max_range = max(MIN_RANGE, new_stats.max_range + (Utils.get_stat("stat_range") / 2)) as int
...
func init_ranged_stats(from_stats: RangedWeaponStats = RangedWeaponStats.new(), weapon_id: String = "", sets: Array = [], effects: Array = [], is_structure: bool = false) -> RangedWeaponStats:
	...
	var piercing_dmg_bonus = (RunData.get_stat("piercing_damage") / 100)
	var bounce_dmg_bonus = (RunData.get_stat("bounce_damage") / 100)
	...
	if from_stats.increase_projectile_speed_with_range:
		new_stats.projectile_speed = clamp(from_stats.projectile_speed + (from_stats.projectile_speed / 300) * Utils.get_stat("stat_range"), 50, 6000) as int
	else:
...
func init_base_stats(from_stats: WeaponStats, weapon_id: String = "", sets: Array = [], effects: Array = [], is_structure: bool = false) -> WeaponStats:
		...
		for set in sets:
			if set.my_id == class_bonus[0]:
				...
				if class_bonus[1] == "lifesteal":
					value = base_stats.get(class_bonus[1]) + (class_bonus[2] / 100)
	...
	for effect in effects:
		if effect is BurningEffect:
			...
			base_stats.set(effect.stat_name, base_stats.get(effect.stat_name) + (effect.value * max(0, nb_same_weapon - 1)))
		elif effect is ExplodingEffect:
			is_exploding = true

	new_stats.scaling_stats = base_stats.scaling_stats

	var atk_spd = (Utils.get_stat("stat_attack_speed") + base_stats.attack_speed_mod) / 100
	...
	new_stats.damage = max(1, new_stats.damage + get_scaling_stats_value(base_stats.scaling_stats)) as int
	...
	if is_exploding:
		exploding_dmg_bonus = (Utils.get_stat("explosion_damage") / 100)
	...
	new_stats.crit_chance = base_stats.crit_chance + (Utils.get_stat("stat_crit_chance") / 100)
	...
	new_stats.lifesteal = ((Utils.get_stat("stat_lifesteal") / 100) + base_stats.lifesteal)

	if is_structure:
		new_stats.lifesteal = base_stats.lifesteal

	new_stats.knockback = max(0, base_stats.knockback + RunData.effects["knockback"]) as int
